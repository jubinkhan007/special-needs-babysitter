...
final sittersListProvider = FutureProvider.autoDispose
    .family<List<SitterListItemModel>, SearchFilterUiModel>((
      ref,
      filters,
    ) async {
      debugPrint(
        'DEBUG: sittersListProvider refreshing with filters: $filters',
      );
      ...
      if (filters.otherLanguage != null &&
          filters.otherLanguage!.trim().isNotEmpty) {
        skills.add(filters.otherLanguage!.trim());
      }

      final maxDistance = filters.radius != defaultFilters.radius
          ? filters.radius.toInt()
          : null;
      final maxRate = filters.hourlyRate != defaultFilters.hourlyRate
          ? filters.hourlyRate
          : null;

      final query = filters.searchQuery?.trim();
      final hasLetters = query != null && RegExp(r'[A-Za-z]').hasMatch(query);
      final hasDigits = query != null && RegExp(r'\d').hasMatch(query);
      final isLocationLike =
          query != null && (query.contains(',') || (!hasLetters && hasDigits));
