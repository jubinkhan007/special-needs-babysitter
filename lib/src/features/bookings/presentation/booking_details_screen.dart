...
class BookingDetailsScreen extends ConsumerStatefulWidget {
  ...
  const BookingDetailsScreen({super.key, required this.args});
...
class _BookingDetailsScreenState extends ConsumerState<BookingDetailsScreen> {
  ...
  Widget build(BuildContext context) {
    final asyncDetails = ref.watch(
      bookingDetailsProvider(widget.args.bookingId),
    );
    ...
    return asyncDetails.when(
      ...
      data: (details) {
        ...
        debugPrint(
          'DEBUG BookingDetailsScreen: ======= BookingDetails received =======',
        );
        debugPrint('DEBUG BookingDetailsScreen: id = ${details.id}');
        debugPrint(
          'DEBUG BookingDetailsScreen: sitterId = ${details.sitterId}',
        );
        debugPrint(
          'DEBUG BookingDetailsScreen: sitterName = "${details.sitterName}"',
        );
        debugPrint(
          'DEBUG BookingDetailsScreen: sitterName length = ${details.sitterName.length}',
        );
        debugPrint(
          'DEBUG BookingDetailsScreen: sitterName contains DioException = ${details.sitterName.contains('DioException')}',
        );
        debugPrint('DEBUG BookingDetailsScreen: status = ${details.status}');
        debugPrint('DEBUG BookingDetailsScreen: skills = ${details.skills}');
        debugPrint(
          'DEBUG BookingDetailsScreen: =====================================',
        );
  ...
  Widget _buildServiceRow(
    ServiceFieldType field,
    BookingDetailsUiModel uiModel,
  ) {
    switch (field) {
      ...
      case ServiceFieldType.numberOfChildren:
        return KeyValueRow(
          ...
          value: uiModel.numberOfChildren,
        );
